...
const registerUser = async (payload) => {
  try {
    const { email, password ,roleType} = payload.body
    ...
    //save password
    const user = await new UserModel({ email, password: hashed_password ,role: roleType}).save();
...
export const userDetail = async (payload) => {
    ...
    try{
        ...
        const currentUserId = await UserModel.findById(userId)
...
export const getUsers = async (payload) => {
  try {
    ...
    const students = UserModel.find({ role: { $ne:ADMIN} })
